...
app.get('/', async (req, res) => {
    ...
    if (!username) {
        return res.status(404).json(response); // Gunakan res.json()
    }
        ...
        if (user) {
            ...
            const data = {
                q: `Buat roasting lucu untuk user @${user.username || ""} berdasarkan data berikut:\n` +
                    ...
                    `Bio: ${user.biography || ""}\n` +
                    `Verifikasi: ${user.is_verified ? "Terverifikasi" : "Tidak terverifikasi"}\n` +
                    `Profile: ${user.is_private ? "Private" : "Publik"}\n` +
